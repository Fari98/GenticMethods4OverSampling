...
def main():
    ...
    estimators = [XGBClassifier(), LogisticRegression()]
    estimator_name = ['w_xgb', 'lr']
    ...
    for j ,estimator in enumerate(tqdm(estimators)):

        for tune in tqdm([True, False]):
            ...
            for i in trange(len(metrics)):
                ...
                for data_name in tqdm(datas):
                    ...
                    if tune and estimator_name[j] == 'lr':
                        pass


                    else:

                        if tune and estimator_name[j] == 'w_xgb':
                            estimator_log_name = 's_xgb'
                            estimator = XGBClassifier(**parameters[data_name])
                            estimator.__name__ = 'StrongClassifier'
                        else:
                            estimator_log_name = estimator_name[j]
                            estimator.__name__ = 'WeakClassifier'
                        ...
                        for _ in trange(30):
                            ...
                            # with open(f'../log/solution_structure_{estimator_name[j]}.csv', 'a', newline='') as file:
