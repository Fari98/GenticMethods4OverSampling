...
from gp4os.utils.functions import TERMINALS, FUNCTIONS
...
def main():
    ...
    for data_name in tqdm([ 'pima-indians-diabetes', 'flare',  'haberman', 'spect', 'spectf', 'ionosphere']):
        ...
        for _ in trange(30):
            ...
            # report_df.to_csv(f'../log/classification_reports/LR_{_}_{data_name}.csv')
            ...
            # report_df = pd.DataFrame(report).transpose()
            # report_df.to_csv(f'../log/classification_reports/LR_SMOTE_{_}_{data_name}.csv')

            with open('../log/baseline_auc.csv', 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(['LR', data_name, _, roc_auc_score(y_test, pred)])
                writer.writerow(['LR_SMOTE', data_name, _, roc_auc_score(y_test, pred_rs)])
            ...
            solver = GP4OS(   pi_eval = pi_eval,
                              ...
                              mutator_tree = mutate_tree_node(4, TERMINALS, FUNCTIONS),
                              crossover_tree = crossover_trees,
